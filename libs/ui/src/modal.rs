        ...
        html! {
            div id=(self.modal_id) tabindex="-1" aria-hidden="true"
                ...
                div class=(format!("relative p-4 w-full {} max-h-full", max_width)) {
                    div class="relative bg-[var(--surface)] rounded-lg shadow-sm" {
                        // Modal header
                        div class="flex items-center justify-between p-4 md:p-5 border-b border-[var(--line)] rounded-t" {
                            h3 class="text-lg font-semibold text-[var(--ink)]" {
                                (self.title)
                            }
                            button type="button" class="text-[var(--muted)] bg-transparent hover:bg-[var(--hover)] hover:text-[var(--ink)] rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-toggle=(self.modal_id) {
                                (Icon::Close)
                        ...
                        @if let Some(footer_content) = &self.footer {
                            div class="flex items-center p-4 md:p-5 border-t border-[var(--line)] rounded-b" {
                                (footer_content)
                            }
                        }
                    }
