...
async fn create_agent(
    ...
    let name = AgentName::from_str(&body.name).map_err(|e| ApiError::Validation(e))?;

    let image_url = ImageUrl::new(format!("user-{}/{}", user_id, body.image))
        .map_err(|e| ApiError::Validation(e))?;

    let agent = state
        .agent_manager
        .create_agent(name, user_id, image_url)
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
